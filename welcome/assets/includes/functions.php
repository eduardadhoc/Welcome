...
function get_countries($siteURL,$mysqli){
	...
	if(mysqli_num_rows($sPaises)>0)
	{
		while($rPaises = $sPaises->fetch_assoc()){
			...
			$html.= '<li class="item '.$rPaises['class'].'"><div class="box">
				<h2 class="title-small">'.$rPaises['name'].'</h2>
				<div class="buttons">';
						...
						while($rLanguages = $sLanguages->fetch_assoc())
						{
							if($rPaises['acronym']!=''){
								...
								if($rLanguages['acronym']!=''){
									...
									$html.= '<a href="#" class="button is-minimal" onClick="return portalizeJS('.$rPaises['id_country'].','.$rLanguages['id_language'].');">'.$rLanguages['name'].'</a>';
								}else{
